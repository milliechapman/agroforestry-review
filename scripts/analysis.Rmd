...
coord <- df_aggregate %>%  group_by(ID, yr, total_words, name) %>% summarise(TF = max(TF)) %>%
  select(ID, yr, total_words, name, TF) %>% drop_na() %>%
  pivot_wider(id_cols = c(yr,ID, total_words), names_from = name, values_from = TF) %>%
  select(agency, access, sovereignty, justice, equity, global, climate,
         farmer, stakeholder,smallholder) 
...
b <- melted_cormat %>%
  filter(!Var1 %in% c("ID","yr", "total_words")) %>%
  filter(!Var2 %in% c("ID","yr", "total_words")) %>%
  mutate(value = ifelse(value<1,value, NA)) %>%
  rename(`co-occurance` = value) %>%
  ggplot(aes(x=Var1, y=reorder(Var2, `co-occurance`), fill=`co-occurance`)) + 
  geom_tile() + theme_classic() +
  scale_fill_gradient(low = "#FFFFE0", high = "#8B0000", na.value = "grey") +
